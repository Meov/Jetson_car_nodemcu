...
void Wifi_server::wifi_init(){
  ...
  int led_satus = 0;
  server.stop(); // msut disable the server before
  if(init_esp)
  {
    WiFi.disconnect();
    SPIFFS.format();
  ...
  if(!wifiManager.autoConnect("AutoConnectAP")){
    ...
    ESP.reset();
    delay(5000);
    //try to connect wifi network until success
  }
  init_esp = false;
...
void Wifi_server::wifi_server() {
  ...
  client.print(WiFi.localIP());
  client.print(F("/gpio/0'>here</a> to switch LED GPIO off.</html>"));  //指向here的超链接 here指向 http://192.168.3.21/gpio/1

  //reset WiFi settings
  client.print(F("<br><br> click <a  href='http://"));
  client.print(WiFi.localIP());
  client.print(F("/wifi_reset'>wifi_reset</a> to reset a wifi network"));
